var load_sidebar = function () {
      ...
      $.each(option_list,function( key, value ) {
             var subopn_html= '<ul class="child">';
             $.each(value.suboptions,function( key, value ) {
                  subopn_html = subopn_html +'<li class=\" \"><a href="'+value.link+'">'+value.name+'</a></li>';
              });
              subopn_html = subopn_html+'</ul>';
             $('#sidebar_menu').append('<li class="parent '+value.color+' active"><a href='+'#'+'><span class="menu-icon"><i class="'+value.icon+'"></i></span><span class="menu-text">'+value.name+'</span><span class="notification">'+value.notifications+'</span></a>'+subopn_html+'</li>');
              
        });
            
            })
          .fail(function() {
            console.log( "json_cannot_be_loaded_from_file" );
          });
    };
  
var load_bar_chart = function (type,title,subtitle,categories,y_title,series) {
...
var load_baseline_chart = function (title, subtitle, categories, y_title,series) {
        $('#baseline_container').highcharts({
            title: {
                text: title,
                x: -20 //center
            },
...
var load_pie_chart = function (title, data) {
    var chart;
    // Build the chart
...
var load_regression_chart = function () {
        $('#regression_container').highcharts({
            chart: {
                zoomType: 'x'
            },
            title: {
                text: 'Total sales from 2012 through 2014'
            },
            subtitle: {
